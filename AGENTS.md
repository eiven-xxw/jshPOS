...
## 4.70 当前 G9A-R3C 报表、终端、商业运营与 POS 辅助页正式整改条件准入

- 项目发起人已于 2026-08-25 接受 G9A-R3C 准备阶段 `CONDITIONAL PASS`，授权从
  `35cc22316faf5389844dfeeee3dfa840ce06cdf8` 建立
  `t2/gate9b-sprint27e-g9a-r3c-runtime`，只整改 `VUE-16..20`、`FLT-01/02/05`
  对应的既有 `G9A-UI-P1-001` 缺口；复用既有 Requirement 和 ADR-071，不新增业务能力或
  Requirement ID。
- 必须按 `R3C-R0` 至 `R3C-R9` 严格串行，先补可复现失败测试，再进行最小页面修复；
  前一步未独立验证，后一步不得铺开。八页必须覆盖路由/导航、按钮、最小权限、可信数据范围、
  加载、空态、安全错误、单航班、离线、业务日、原幂等键恢复和直接组件/Widget 测试。
- 五个 Vue 页面必须由服务端执行最终授权，导出、附件、凭据轮换、状态迁移等高风险写操作
  必须二次确认并在明确成功前复用原操作身份；客户端 tenant_id、门店、业务日或 Owner 结果
  不得作为权威事实。
- 三个 Flutter 辅助页只能通过既有应用服务和正式 Owner 编排端口操作；换货继续表达为原单退货
  加新销售，退货退款继续读取原成交快照，现金钱箱的真实设备执行继续失败关闭。UNKNOWN、离线、
  ACK 丢失和重启恢复必须观察原命令，禁止生成替代业务或资金命令。
- 本批禁止修改服务端 Controller/API、Owner 领域逻辑、依赖和已发布 MySQL/SQLite 迁移；终端
  客户端 API 仅允许增加向后兼容的可选原幂等键参数，不改变服务端端点或契约。
- 本批完成后只能形成 `G9A-R3C VERIFIED_CANDIDATE`；`G9A-UI-P1-001` 继续 `OPEN`，未经项目
  发起人确认不得进入 R3D 联合验收、R4、完整 Alpha、现场试点或生产发布。
- `T2-PAY-002/HWD-001/PRN-001/PAR-001` 继续 `BLOCKED`；`T2-UAT-001/REL-001`
  继续 `DRAFT`；`T2-LIC-001/JSH-001` 继续 `DEFERRED`。Provider 网络、真实资金、
  设备/外设命令、伙伴现场、完整 Alpha、生产部署和商业声明必须为 0。

## 4.71 当前 G9A-R3D 全部 26 页联合验收准备阶段条件准入

- 项目发起人已于 2026-08-25 接受 G9A-R3C `CONDITIONAL PASS`；接受范围仅覆盖
  `VUE-16..20`、`FLT-01/02/05` 的内部页面证据，整体 `G9A-UI-P1-001` 继续
  `OPEN`。本阶段从 G9A-R3C 最终治理提交
  `f5909b6094d004e4320d3afaa1dce7a1f170485b` 建立
  `t2/gate9b-sprint27f-g9a-r3d-prep`。
- 本阶段只允许汇总 R3A、R3B、R3C 已封存的 20 个 Vue 与 6 个 Flutter 页面证据，核验
  路由/导航、按钮、最小权限、可信数据范围、加载、空态、安全错误、单航班、离线、业务日、
  原幂等键恢复、危险操作和直接组件/Widget 测试；不得修改页面运行时、服务端、API、
  Controller、Owner 领域代码、依赖、数据库、任务或已发布迁移。
- 历史批次契约、报告、失败 Run 和验收结论必须保持不可变。联合层只能保存来源引用、摘要、
  跨页旅程、联合失败 seed、验收矩阵和待批修复计划，不得把单页 `PASS` 自动推导为跨页
  `PASS`，不得用静态字符串断言冒充路由挂载、组件交互或 Widget 交互。
- 必须至少复跑 Web 正式构建、Lint、类型检查和页面组件回归，以及 Flutter 双平台格式、
  Analyze、Widget 回归和覆盖率；联合旅程须覆盖后台主数据到经营报表、SaaS 到订阅与服务、
  可信 POS 会话到销售/组合支付/现金/退货/换货三条链路。跨客户端证据只能标记为
  `INTERNAL_UI_EVIDENCE_CHAIN`，不得冒充完整 E2E、Alpha 或现场证据。
- 准备阶段可以登记 P0/P1 联合证据缺口并冻结后续串行顺序，但不得修复页面或新增测试运行时；
  `G9A-UI-P1-001` 必须继续 `OPEN`。未经项目发起人再次确认不得进入 R3D 正式联合整改、
  G9A-R4、完整 Alpha、现场试点或生产发布。
- `T2-PAY-002/HWD-001/PRN-001/PAR-001` 继续 `BLOCKED`；`T2-UAT-001/REL-001`
  继续 `DRAFT`；`T2-LIC-001/JSH-001` 继续 `DEFERRED`。Provider 网络、真实资金、
  设备/外设命令、伙伴现场、完整 Alpha、生产部署和商业声明必须为 0。
...
## 4.84 当前 T2 Gate 10A-R2-R2-R2-R0/R1 RPT-SALES 精确整改条件准入

- 项目发起人已于 2026-08-26 接受精确整改准备阶段 `CONDITIONAL PASS` 及
  `CR-T2G10A-013/014/015` 的兼容性方向，授权从准备阶段最终封存提交
  `972fcfafdf131a555e47f5644e0ae914ce6297fb` 建立
  `t2/gate10a-r2-r2-r2-r0-r1-rpt-sales-runtime`；本批运行时只准入 Reporting 批量读取端口、
  `RPT-SALES` 版本化 keyset 分页和受控流式导出，`RPT-INVENTORY/RPT-PAY-REC` 不得提前整改。
- 必须先把 `R2R2R2-F01/F04/F07/F08/F09/F12` 建成可复现失败测试并保留红基线，再做最小实现。
  其中跨三报表或全部候选的父 Seed 在本批只能记录 RPT-SALES 子范围进展，不得伪造为整体关闭。
- 旧 `/api/v1/reports/sales-daily`、既有 operationId、响应和错误码保持冻结兼容窗口；新客户端只能
  使用独立版本化入口、HMAC 签名且绑定可信租户/投影/筛选的游标。`tenant_id` 不得来自客户端，
  门店范围必须逐项通过服务端数据权限校验。
- Reporting Owner 批量端口只读取自有投影；复杂 keyset、多门店集合和执行计划敏感 SQL 必须位于
  Mapper XML。交互页必须有界，导出按固定批次流式写入，字节偏移、游标和请求摘要共同形成恢复
  检查点；同导出身份异摘要、游标篡改、跨租户和恢复缺口必须失败关闭。
- 旧列表契约和库存/支付报表导出必须保持兼容，不得为了查询数归零提前删除或改写。R1 只允许
  记录 RPT-SALES 对 `150` 查询放大的贡献下降，父 Seed 继续等待后续获批批次。
- 必须在 MySQL 8.4.11 的固定 10k/100k 合成分布保存正式 SQL 摘要、参数、`SHOW INDEX`、
  `EXPLAIN JSON/TREE`、查询次数、完整性、租户攻击和计划建议；不得用墙钟波动单独判定。
- 若执行计划证明需要新增或调整索引，必须停止索引实施并提交独立索引 CR 与唯一前向迁移方案；
  未经项目发起人确认不得修改索引、数据库对象或迁移。任何已发布迁移均不得修改。
- 当前 MySQL 8.4.11 的固定 10k/100k 执行结果均为 0 重复、0 缺失、0 跨租户且金额守恒，
  但都观察到全表扫描与 filesort；已形成 `CR-T2G10A-018`，候选只允许通过唯一
  `V202608260088` 新增 `idx_rpt_sales_keyset`。在项目发起人明确确认前，迁移与索引变化必须为 0。
- 本批不得新增未批准业务能力或 Requirement ID，不得改变资金、库存、租户、支付、同步、事件
  Schema 或 Owner 事实。`G10A-SQL-P2-001` 继续 `OPEN`，`G10A-RES-P2-001` 继续 `PREPARED`；
  未经周门禁确认不得进入后续查询、RES、R3、完整 Alpha 或生产发布。
- 所有外部 `BLOCKED`、UAT/REL `DRAFT`、LIC/JSH `DEFERRED` 与 Provider 网络、真实资金、
  设备/外设、伙伴现场、完整 Alpha 和生产部署零执行边界保持不变。
...
## 4.87 当前 T2 Gate 10A-R2-R2-R2-R2 RPT-INVENTORY 正式运行时精确整改条件准入

- 项目发起人已于 2026-08-27 接受 RPT-INVENTORY 准备阶段 `CONDITIONAL PASS`，并授权从唯一
  准备封存提交 `f36df63b21bd3bb98ea0d5022f8fe5fac5def72f` 建立
  `t2/gate10a-r2-r2-r2-r2-rpt-inventory-runtime`。
- 本批只允许复用 `T2-RPT-001`、`T2-API-001`、`CR-T2G10A-014` 和 `ADR-074`，先固化失败回归，
  再串行实现 `ReportingBatchReadPort.readInventoryCost`、版本化 keyset、HMAC 签名游标、受控流式
  导出与原身份恢复；v1 API 与 Mapper 查询保持冻结兼容。
- `tenant_id`、投影版本与门店范围只能由可信服务端上下文注入；游标必须绑定租户、筛选摘要和投影
  版本，同键异内容、坏游标、跨租户游标、越权门店与摘要漂移必须失败关闭。
- 必须在 MySQL 8.4.11 固定 10k/100k 分布验证查询数、执行计划、0 重复/缺失/越权和十二项数量/
  成本字段守恒。若候选 SQL 需要新增或调整索引，立即停止并提交独立索引 CR 与唯一前向迁移，
  未经确认不得实施索引或迁移。
- 不得改变库存、成本、租户、同步、API v1 或事件语义，不得修改任何已发布迁移；RPT-PAY-REC、
