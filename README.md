...
当前阶段：**T2 Gate 7B / Sprint S20-B：T2-EXG-001 独立正式开发**。只允许实现“原单退货退款 + 新销售 + 只追加关联”的基础换货 Saga、前向迁移和 Flutter 旅程；`T2-PAY-004` 与 Gate 7C 继续 `DRAFT`。支付沙箱、真实硬件、真实打印和设计伙伴继续 `BLOCKED`，完整 Alpha 与发布继续 `DRAFT`；不得宣称可试点、可生产或可商用。
