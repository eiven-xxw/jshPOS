...
public class SubscriptionIdGenerator {
    ...
    public SubscriptionIdGenerator(Clock clock) { this.clock = clock; }

    public synchronized String next() {
        long timestamp = clock.millis();
        if (timestamp < 0 || timestamp > 0xFFFFFFFFFFFFL) throw new IllegalStateException("SUB-ID-001: ULID 时间越界");
