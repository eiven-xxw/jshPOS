...
public class TransferService {
    ...
    private final TransferCommandPolicy commandPolicy = new TransferCommandPolicy();

    /** 创建不产生数量或成本效果的调拨草稿，并冻结基础单位换算。 */
    @Transactional
    public TransferDetail create(CreateTransfer command) {
        commandPolicy.validateCreate(command);
        ...
        String requestHash = commandPolicy.hashCreate(command);
        ...
        for (CreateLine line : command.lines().stream().sorted(Comparator.comparing(CreateLine::transferLineId)).toList()) {
            ...
            BigDecimal base = commandPolicy.toBase(input, unit);
    ...
    public TransferDetail dispatch(DispatchTransfer command) {
        commandPolicy.validateDispatch(command);
        TrustedPrincipal principal = tenantContext.requirePrincipal();
        TransferHead head = requireLocked(principal.tenantId(), command.transferId());
        authorizeBoth(head);
        String hash = commandPolicy.hashDispatch(command);
    ...
    public TransferDetail receive(ReceiveTransfer command) {
        commandPolicy.validateReceive(command);
        TrustedPrincipal principal = tenantContext.requirePrincipal();
        TransferHead head = requireLocked(principal.tenantId(), command.transferId());
        authorizeBoth(head);
        String hash = commandPolicy.hashReceive(command);
    ...
    public TransferDetail resolveDifference(ResolveDifference command) {
        commandPolicy.validateDifference(command);
        TrustedPrincipal principal = tenantContext.requirePrincipal();
        TransferHead head = requireLocked(principal.tenantId(), command.transferId());
        authorizeBoth(head);
        String hash = commandPolicy.hashDifference(command);
    ...
    public TransferDetail cancel(StateCommand command) {
        commandPolicy.validateState(command);
        TrustedPrincipal principal = tenantContext.requirePrincipal();
        TransferHead head = requireLocked(principal.tenantId(), command.transferId());
        authorizeBoth(head);
        Status current = Status.valueOf(head.status());
        ...
        requireVersion(head, command.expectedVersion());
        String hash = commandPolicy.stateHash(command, "CANCEL");
    ...
    private TransferDetail transition(StateCommand command, Status expected, Status next,
        ...
        TransferHead head = requireLocked(principal.tenantId(), command.transferId());
        authorizeBoth(head);
        String hash = commandPolicy.stateHash(command, action);
