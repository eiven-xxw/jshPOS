...
class PosPromotionSnapshotServiceTest {
    ...
    private static final String SETTLEMENT_FINGERPRINT = "3".repeat(64);
    ...
    void ingestsOfflineQuoteAndExactSnapshotThroughPromotionOwner() {
        ...
        ArgumentCaptor<PromotionPersistencePort.SnapshotWrite> snapshot =
            ArgumentCaptor.forClass(PromotionPersistencePort.SnapshotWrite.class);
        verify(persistence).insertSnapshot(snapshot.capture());
        assertThat(snapshot.getValue().quoteFingerprint()).isEqualTo(SETTLEMENT_FINGERPRINT);
    ...
    private SnapshotCommand command(String hash) {
        return new SnapshotCommand(EVENT, CORRELATION, QUOTE, SNAPSHOT, ORDER, 1101L, TERMINAL,
            LocalDate.parse("2026-08-16"), 1, "promotion-engine-1.0.0", FINGERPRINT,
            SETTLEMENT_FINGERPRINT, hash,
