...
| `promotion:manual:authorize` | 手工改价、整单优惠和抹零 | 门店、员工阈值与独立审批 |
...
正式 OpenAPI 位于 `contracts/t2/gate5a/openapi-promotion-v1.yaml`，包括规则创建/预检/审批/发布/暂停、规则包获取、报价、人工优惠授权、快照冻结和退款恢复查询。请求 DTO 不接收可作为授权依据的 tenant_id。
...
正式事件为 `promotion.rule.published.v1`、`promotion.rule.paused.v1`、`promotion.package.published.v1`、`promotion.quote.calculated.v1`、`promotion.snapshot.frozen.v1`、`promotion.refund.allocation-recorded.v1`。事件使用稳定 event_id、聚合版本、payload hash、correlation_id 和可信 tenant_id；消费者至少一次接收、Inbox 防重。

事件只携带必要标识、版本、摘要和金额汇总，不携带生产密钥、真实会员敏感信息、完整规则签名私钥或支付报文。

## 4. 离线规则包

规则包包含：`tenantId`、`storeId`、`packageVersion`、`promotionReleaseVersion`、`engineCompatibility`、`schemaVersion`、规则 AST、适用索引、业务时区、有效期、撤销版本、manifest SHA-256 和平台签名。

POS 安装顺序：写入非活动槽 → 验证可信设备 tenant/store → 验证 Schema/引擎能力 → 规范化规则并复算摘要 → 验证测试签名 → 执行包内自检向量 → SQLite 事务原子切换 active slot。任何失败都不能改变活动指针；活动包过期、撤销或包含未支持算子时促销失败关闭并显示明确原因。
