...
历史 V1—V4 字符串和校验和不得修改。V5 必须支持 V1→V5 连续升级、空库、含挂单/完成单、进程终止前后和重复启动恢复测试；成交快照、行分摊和退款账本通过可信设备复合外键、金额守恒 CHECK、正数精确数量 CHECK 和禁止 UPDATE/DELETE 触发器失败关闭。
