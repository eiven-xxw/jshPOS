...
待证伪假设：十家支付/聚合服务候选可以先用统一 Provider Profile 描述，从中评选出的五家公开能力可映射到统一的“命令 + 查询 + 回调 + 对账”语义；客户端超时不会触发第二次扣款，所有 `UNKNOWN` 最终仅通过原交易查询、回调或对账收敛。
...
### 4.3 十家档案、五家 Fake 与一个真实沙箱

- 易宝、汇付、拉卡拉、富友、收钱吧、银联商务、通联、嘉联、杉德、随行付：建立 Provider Profile、公开资料、能力和缺口；
- 按文档完整度、UNKNOWN/查询/退款/回调/对账、沙箱和支持能力选出五家建立字段映射及可脚本化 Fake，共用一套 provider contract test；
...
禁止真实金额、生产商户号、生产终端、生产私钥和银行卡数据。真实沙箱账号/终端/文档/联系人未提供，`T1-PAY-002` 为 `BLOCKED`；十家档案和评选出的五家 Fake 契约已获 Week 1 条件准入。
