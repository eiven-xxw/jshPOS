#!/usr/bin/env python3
"""聚合 G9A-R3D 多执行器证据并生成不可变 SHA-256 索引。"""
...
def main() -> int:
    ...
    if missing:
        raise AssertionError(f"缺少 G9A-R3D 证据生产者: {missing}")
    files = [
        {
    ...
    if not files:
        raise AssertionError("G9A-R3D 证据包为空")
    result = {
        "schemaVersion": "1.0",
        "gate": "T2-GATE9B-G9A-R3D",
        "findingId": "G9A-UI-P1-001",
        "batchState": "CONDITIONAL_PASS_CANDIDATE",
        "overallFindingState": "OPEN_AWAITING_SPONSOR_CONFIRMATION",
        "evidenceLevel": "INTERNAL_SOFTWARE_UI_JOINT_ACCEPTANCE",
    ...
    print(f"G9A-R3D EVIDENCE OK: producers={len(EXPECTED_PRODUCERS)} files={len(files)}")
    return 0
