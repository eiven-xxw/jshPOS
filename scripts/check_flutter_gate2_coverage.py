...
def main() -> None:
    ...
    if not selected:
        raise SystemExit("FLUTTER COVERAGE ERROR: no Gate 2 runtime files found")
    found, covered = sum(value[0] for value in selected.values()), sum(value[1] for value in selected.values())
    ratio = covered / found
    if ratio < args.minimum:
        raise SystemExit(f"FLUTTER COVERAGE ERROR: {ratio:.4f} below {args.minimum:.4f}")
    report = {
        "schemaVersion": "1.0",
        "scope": "T2 Gate 2 Flutter checkout, shift and SQLite runtime",
    ...
    print(f"FLUTTER GATE2 COVERAGE OK: lines={covered}/{found} ratio={ratio:.4f} minimum={args.minimum:.2f}")
