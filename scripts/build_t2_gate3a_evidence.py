...
def fail(message: str) -> None:
    raise SystemExit(f"T2-GATE3A EVIDENCE ERROR: {message}")
...
def jacoco(root: Path) -> dict[str, float | int]:
    reports = [path for path in root.rglob("jacoco.xml") if "jshpos-payment" in path.as_posix()]
    if len(reports) != 1:
        fail(f"expected one payment JaCoCo report, got {len(reports)}")
    ...
    for name, minimum in (("LINE", 0.90), ("BRANCH", 0.85)):
        ...
        if counter is None:
            fail(f"payment JaCoCo missing {name}")
        missed, covered = int(counter["missed"]), int(counter["covered"])
        ratio = covered / (covered + missed)
        if ratio < minimum:
            fail(f"payment JaCoCo {name} {ratio:.4f} below {minimum:.2f}")
...
def requirements(stage: str) -> dict[str, object]:
    ...
    for item in ("T2-PAY-001", "T2-PAY-003", "T2-REF-001", "T2-REC-001"):
...
def main() -> None:
    ...
    directories = ("governance", "server", "mysql", "tenant", "fake", "web", "pos-linux", "pos-windows", "security")
    ...
    fake_reports = list((bundle / "fake").rglob("fake-matrix.json"))
    if len(fake_reports) != 1:
        fail("fixed Fake vector matrix is missing")
    fake = json.loads(fake_reports[0].read_text(encoding="utf-8"))
    if fake.get("vectorCount") != 16 or fake.get("providerNetworkCalls") != 0:
        fail("fixed Fake vector matrix is incomplete or network boundary changed")
    ...
    for path in sorted(bundle.rglob("*")):
        if path.is_file() and path.name != "t2-gate3a-evidence-index.json":
            files.append({"path": path.relative_to(bundle).as_posix(), "size": path.stat().st_size, "sha256": digest(path)})
    index = {
        "schemaVersion": "1.0", "phase": "T2-GATE3A", "stage": args.stage,
        "evidenceLevel": "STATIC+UNIT+MYSQL_INTEGRATION+FLUTTER_REGRESSION+FAKE_CONTRACT",
        "generatedAt": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"),
        "baselineTag": "t2-prep-baseline-2026-08-16",
        "branchStart": "60828acbbe18c30914a076709905eeb525ffeb5f",
        ...
        "tenantAttackSurfaces": 12, "fixedFakeVectors": 16, "providerNetworkCalls": 0,
        "apk": {"size": apk[0].stat().st_size, "sha256": digest(apk[0])},
        "externalEvidence": {"sandbox": 0, "realDevice": 0, "pilot": 0},
        "files": files,
        "limitations": [
            "Payment, refund and reconciliation evidence uses two fictional tenants and synthetic facts",
            "Android physical power-loss, REAL_DEVICE, payment SANDBOX and PILOT evidence remain zero",
            "Gate 3A runtime is Provider-neutral; no Provider adapter, callback endpoint, credential or network call is present",
            "Fake contract vectors do not establish SANDBOX or real-funds evidence",
            "This evidence does not establish Alpha, pilot readiness, or commercial acceptance",
        ],
    }
    (bundle / "t2-gate3a-evidence-index.json").write_text(
        json.dumps(index, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(f"T2-GATE3A EVIDENCE OK: stage={args.stage} files={len(files)} serverTests={tests['server']['tests']} "
