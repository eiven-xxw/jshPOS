...
def fail(message: str) -> None:
    raise SystemExit(f"T2-GATE4B EVIDENCE ERROR: {message}")
...
def requirements(stage: str) -> dict[str, object]:
    ...
    for item in ("T2-INV-003", "T2-PUR-001"):
        if rows[item]["status"] != expected:
            fail(f"{item} must be {expected}")
    for item in ("T2-CST-001", "T2-TRF-001"):
...
def main() -> None:
    ...
    tests = {
        "server": surefire(bundle / "server"),
        "mysql": surefire(bundle / "mysql"),
        "tenant": surefire(bundle / "tenant"),
        "flutterLinux": flutter_tests(bundle / "pos-linux"),
        "flutterWindows": flutter_tests(bundle / "pos-windows"),
    }
    server_coverage = {
        "inventory": jacoco(bundle / "server", "jshpos-inventory"),
        "procurement": jacoco(bundle / "server", "jshpos-procurement"),
    }
    ...
    if len(attack.get("surfaces", [])) != 14 or any(x.get("result") != "PASS" for x in attack["surfaces"]):
    ...
    if vectors.get("vectorCount") != 20 or vectors.get("providerNetworkCalls") != 0:
    ...
    files = [
        ...
        if path.is_file() and path.name != "t2-gate4b-evidence-index.json"
    ]
    index = {
        "schemaVersion": "1.0",
        "phase": "T2-GATE4B",
        ...
        "branchStart": "9557112a4eb573ec4a54ef30be477f1ab8f09d31",
        "commitSha": subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=ROOT, text=True).strip(),
        "requirements": requirements(args.stage),
        "tests": tests,
        "coverage": {"server": server_coverage, "flutter": flutter_coverage},
        "tenantAttackSurfaces": 14,
        "fixedVectors": 20,
        ...
        "files": files,
        "limitations": [
            "Evidence uses two fictional tenants and synthetic stocktake/procurement data",
            "T2-CST-001 and T2-TRF-001 remain design-only DRAFT with no runtime",
    ...
    (bundle / "t2-gate4b-evidence-index.json").write_text(
        json.dumps(index, ensure_ascii=False, indent=2) + "\n", encoding="utf-8"
    )
    print(
        f"T2-GATE4B EVIDENCE OK: stage={args.stage} files={len(files)} "
        f"serverTests={tests['server']['tests']} vectors=20 paymentNetwork=0"
    )
