...
PRIOR_ACCEPTED = {
    ...
    "T2-POS-004", "T2-POS-005", "T2-ORD-001", "T2-ORD-002", "T2-OFF-001",
}


def fail(message: str) -> None:
    raise SystemExit(f"T2-SPRINT3 EVIDENCE ERROR: {message}")
...
def requirements(stage: str) -> dict[str, object]:
    ...
    if rows["T2-SYN-001"]["status"] != expected:
        fail(f"T2-SYN-001 must be {expected}")
    ...
    return {"priorAccepted": len(PRIOR_ACCEPTED), "sync": expected, "gate3": "DRAFT", "paymentSandbox": "BLOCKED"}
...
def main() -> None:
    ...
    directories = ("governance", "server", "mysql", "tenant", "fault", "web", "pos-linux", "pos-windows", "security")
    ...
    faults = list((bundle / "fault").rglob("fault-matrix.json"))
    if len(faults) != 1 or json.loads(faults[0].read_text(encoding="utf-8")).get("seedCount") != 14:
        fail("fixed fault seed matrix is incomplete")
    ...
    for path in sorted(bundle.rglob("*")):
        if path.is_file() and path.name != "t2-sprint3-evidence-index.json":
    ...
    index = {
        ...
        "evidenceLevel": "STATIC+UNIT+MYSQL_INTEGRATION+SQLITE_FAULT+LOOPBACK_HTTP",
        "generatedAt": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"),
        "baselineTag": "t2-prep-baseline-2026-08-16",
        "branchStart": "968ae7be34ab144c970e5c92fb7ffbddf60bf5e1",
        ...
        "tenantAttackSurfaces": 12, "fixedFaultSeeds": 14,
        "apk": {"size": apk[0].stat().st_size, "sha256": digest(apk[0])},
        "externalEvidence": {"sandbox": 0, "realDevice": 0, "pilot": 0},
        "files": files,
        "limitations": [
            "Synchronization evidence uses two fictional tenants and synthetic business facts",
            "Android physical power-loss, REAL_DEVICE, payment SANDBOX and PILOT evidence remain zero",
            "Gate 3 payment/refund/reconciliation artifacts are design-only and no provider network call is present",
            "This evidence does not establish Alpha, pilot readiness, or commercial acceptance",
        ],
    }
    (bundle / "t2-sprint3-evidence-index.json").write_text(
        json.dumps(index, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(f"T2-SPRINT3 EVIDENCE OK: stage={args.stage} files={len(files)} serverTests={tests['server']['tests']} "
