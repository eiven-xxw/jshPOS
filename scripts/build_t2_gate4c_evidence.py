...
def fail(message: str) -> None:
    raise SystemExit(f"T2-GATE4C EVIDENCE ERROR: {message}")
...
def requirements(stage: str) -> dict[str, object]:
    ...
    if rows["T2-CST-001"]["status"] != expected:
        fail(f"T2-CST-001 must be {expected}")
    if rows["T2-TRF-001"]["status"] != "DRAFT":
        fail("T2-TRF-001 must remain DRAFT")
...
def main() -> None:
    ...
    coverage = {module: jacoco(bundle / "server", f"jshpos-{module}")
                for module in ("inventory", "procurement", "costing")}
    ...
    if len(attack.get("surfaces", [])) != 15 or any(x.get("result") != "PASS" for x in attack["surfaces"]):
    ...
    if vectors.get("vectorCount") != 24 or vectors.get("providerNetworkCalls") != 0:
    ...
    files = [{"path": path.relative_to(bundle).as_posix(), "size": path.stat().st_size,
             ...
             if path.is_file() and path.name != "t2-gate4c-evidence-index.json"]
    index = {
        ...
        "branchStart": "02e1854a9b2a63a46323f27457bd61a708f740a9",
        ...
        "tenantAttackSurfaces": 15, "fixedVectors": 24, "providerNetworkCalls": 0,
        ...
        "limitations": [
            ...
            "T2-TRF-001 remains design-only DRAFT with no transfer runtime",
    ...
    (bundle / "t2-gate4c-evidence-index.json").write_text(
        json.dumps(index, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(f"T2-GATE4C EVIDENCE OK: stage={args.stage} files={len(files)} "
          f"serverTests={tests['server']['tests']} vectors=24 paymentNetwork=0")
