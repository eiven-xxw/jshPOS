...
def main() -> None:
    ...
    mysql_result = next((path for path in files if path.name == "rpt-pay-rec-red-baseline.json"), None)
    export_result = next((path for path in files if path.name == "legacy-export-query-count.json"), None)
    reference_result = next((path for path in files if path.name == "payment-reference-query-count.json"), None)
    if mysql_result is None or export_result is None or reference_result is None:
        raise SystemExit("missing RPT-PAY-REC MySQL red baseline evidence")
    ...
    if any(len(row["differenceInvariants"]) != 12 for row in rows):
        raise SystemExit("difference invariant count mismatch")
    payload = {
        "schemaVersion": "1.0",
