#!/usr/bin/env python3
"""聚合 Gate 7F-Prep 双平台离线证据并生成不可变 SHA-256 索引。"""
...
GATE = "T2-GATE7F-PREP-EXTERNAL-P0-LICENSE-ALPHA-ADMISSION"
...
def main() -> None:
    ...
    if missing:
        raise SystemExit(f"missing Gate 7F evidence producers: {missing}")
    reports = []
    for path in files:
        if path.suffix == ".json":
            try:
    ...
    if len(reports) < 3 or any(report.get("status") != "PASS" for report in reports):
        raise SystemExit("Gate 7F producer reports missing or non-green")
    for report in reports:
        if any(value != 0 for value in report.get("externalExecution", {}).values()):
            raise SystemExit("Gate 7F external execution boundary violated")
        if report.get("overallDecision") != "PREPARED_NO_GO_EXTERNAL_EXECUTION_FULL_ALPHA_AND_RELEASE":
            raise SystemExit("Gate 7F NO-GO boundary drift")
    ...
    result = {
        ...
        "evidenceLevel": "STATIC_GOVERNANCE_AND_OFFLINE_METADATA",
        ...
        "externalExecution": {"providerNetworkCalls":0,"realFundsTransactions":0,"realDeviceCommands":0,
                              "realPeripheralCommands":0,"partnerContacts":0,"onsitePilots":0,
                              "fullAlphaRuns":0,"productionDeployments":0,"commercialClaims":0},
        "limitations": ["PAY 0/11", "HWD 0/2", "PRN 0/6", "PAR 0/5 and written intent 0/3",
                        "LIC 0/3", "No SANDBOX REAL_DEVICE REAL_PERIPHERAL PILOT FULL_ALPHA or PRODUCTION"]
    }
