...
def main() -> None:
    ...
    mysql_result = next((path for path in files if path.name == "rpt-sales-keyset-results.json"), None)
    if mysql_result is None:
        raise SystemExit("missing MySQL RPT-SALES result")
    plans = json.loads(mysql_result.read_text(encoding="utf-8"))
    index_required = any(item["indexCrRequired"] for item in plans)
    payload = {
        "schemaVersion": "1.0",
        "gate": "T2_GATE10A_R2_R2_R2_R0_R1_RPT_SALES",
        "commit": args.commit,
        "decision": "CONDITIONAL_NO_GO_PENDING_INDEX_CR" if index_required
                    else "R0_R1_VERIFIED_AWAITING_SPONSOR_CONFIRMATION",
        "findingState": "OPEN",
