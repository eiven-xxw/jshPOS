...
PREFIXES = (
    "lib/features/return_refund/domain/",
    "lib/features/return_refund/application/",
    "lib/features/return_refund/infrastructure/",
)
...
def main() -> None:
    ...
    if not selected:
        raise SystemExit("FLUTTER GATE6E COVERAGE ERROR: no return/refund runtime files found")
    found = sum(value[0] for value in selected.values())
    covered = sum(value[1] for value in selected.values())
    ratio = covered / found
    if ratio < args.minimum:
        raise SystemExit(f"FLUTTER GATE6E COVERAGE ERROR: {ratio:.4f} below {args.minimum:.4f}")
    report = {
        "schemaVersion": "1.0",
        "scope": "T2-POS-009 return/refund domain and application runtime",
    ...
    print(f"FLUTTER GATE6E COVERAGE OK: lines={covered}/{found} ratio={ratio:.4f}")
