#!/usr/bin/env python3
"""生成 T2-DMT-001 跨平台 SHA-256 证据索引。"""
...
def main() -> None:
    ...
    if missing:
        raise SystemExit(f"missing DMT001 evidence producers: {missing}")
    reports = []
    for path in files:
        if path.suffix != ".json":
            continue
    ...
    if len(reports) < 2 or any(report.get("status") != "PASS" for report in reports):
        raise SystemExit("Ubuntu/Windows DMT001 governance reports missing or non-green")
    ...
    if len(statuses) != 1 or not statuses.issubset({"IN_PROGRESS", "VERIFIED"}):
        raise SystemExit("DMT001 requirement status missing or inconsistent")
    status = statuses.pop()
    for report in reports:
        ...
        if preserved.get("T2-ONB-001") != "DRAFT" or preserved.get("T2-PAY-002") != "BLOCKED":
    ...
    result = {
        ...
        "decision": "DMT001_VERIFIED_AWAITING_SPONSOR_ACCEPTANCE" if status == "VERIFIED"
                    else "DMT001_CANDIDATE_GREEN_AWAITING_RTM_VERIFICATION",
        "requirementStatus": status, "producers": sorted(producers), "fileCount": len(entries), "files": entries,
        "limitations": [
            "Raw files and secrets are not evidence artifacts; only hashes and masked results are indexed.",
            "100k/1m results are internal synthetic budgets and not production capacity or SLA evidence.",
            "T2-DMT-001 is not ACCEPTED until project sponsor confirmation."
        ],
    }
