#!/usr/bin/env python3
"""聚合 G9A-R3C 多执行器证据并生成 SHA-256 索引。"""
...
def main() -> int:
    ...
    if missing:
        raise AssertionError(f"缺少 G9A-R3C 证据生产者: {missing}")
    files = [
        {"path": path.relative_to(bundle).as_posix(), "size": path.stat().st_size, "sha256": digest(path)}
        for path in sorted(item for item in bundle.rglob("*") if item.is_file())
    ]
    if not files:
        raise AssertionError("G9A-R3C 证据包为空")
    result = {
        "schemaVersion": "1.0",
        "gate": "T2-GATE9B-G9A-R3C",
    ...
    print(f"G9A-R3C EVIDENCE OK: producers={len(EXPECTED_PRODUCERS)} files={len(files)}")
    return 0
