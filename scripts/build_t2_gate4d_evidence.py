...
def fail(message: str) -> None:
    raise SystemExit(f"T2-GATE4D EVIDENCE ERROR: {message}")
...
def jacoco(root: Path, module: str) -> dict[str, float | int]:
    reports = [path for path in root.rglob("jacoco.xml") if module in path.as_posix()]
    if len(reports) != 1:
        fail(f"expected one {module} JaCoCo report, got {len(reports)}")
    ...
    for name, minimum in (("LINE", 0.90), ("BRANCH", 0.85)):
        ...
        if counter is None:
            fail(f"{module} JaCoCo missing {name}")
        missed, covered = int(counter["missed"]), int(counter["covered"])
        ratio = covered / (covered + missed)
        if ratio < minimum:
            fail(f"{module} JaCoCo {name} {ratio:.4f} below {minimum:.2f}")
...
def requirements(stage: str) -> dict[str, object]:
    ...
    if rows["T2-TRF-001"]["status"] != expected:
        fail(f"T2-TRF-001 must be {expected}")
    if any(rows[item]["status"] != "DRAFT" for item in ("T2-PRM-001", "T2-PRM-002", "T2-PRM-003")):
        fail("T2-PRM-001..003 must remain DRAFT")
...
def main() -> None:
    ...
    coverage = {module: jacoco(bundle / "server", f"jshpos-{module}")
                for module in ("inventory", "procurement", "transfer", "costing")}
    ...
    if flutter_coverage.get("lineRatio", 0) < 0.90:
        fail("Flutter line coverage below 90%")
    ...
    if len(attack.get("surfaces", [])) != 18 or any(x.get("result") != "PASS" for x in attack["surfaces"]):
    ...
    if vectors.get("vectorCount") != 28 or vectors.get("providerNetworkCalls") != 0:
    ...
    files = [{"path": path.relative_to(bundle).as_posix(), "size": path.stat().st_size,
             ...
             if path.is_file() and path.name != "t2-gate4d-evidence-index.json"]
    index = {
        ...
        "branchStart": "b84555ee7af02c650a47146426f6c9e4029827b5",
        ...
        "coverage": {"server": coverage, "flutter": flutter_coverage},
        "tenantAttackSurfaces": 18, "fixedVectors": 28, "providerNetworkCalls": 0,
        "apk": {"size": apk[0].stat().st_size, "sha256": digest(apk[0])},
        "externalEvidence": {"sandbox": 0, "realDevice": 0, "pilot": 0}, "files": files,
        "limitations": [
            "Evidence uses two fictional tenants and synthetic cost facts",
            "T2-PRM-001..003 remain design-only DRAFT with no promotion runtime",
    ...
    (bundle / "t2-gate4d-evidence-index.json").write_text(
        json.dumps(index, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(f"T2-GATE4D EVIDENCE OK: stage={args.stage} files={len(files)} "
          f"serverTests={tests['server']['tests']} vectors=28 paymentNetwork=0")
