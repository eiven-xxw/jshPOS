#!/usr/bin/env python3
"""生成 T2-RPL-001 跨平台 SHA-256 证据索引。"""
...
def main() -> None:
    ...
    if missing:
        raise SystemExit(f"missing RPL001 evidence producers: {missing}")
    reports = []
    for path in files:
        if path.suffix != ".json":
            continue
    ...
    if len(reports) < 2 or any(report.get("status") != "PASS" for report in reports):
        raise SystemExit("Ubuntu/Windows RPL001 governance reports missing or non-green")
    ...
    if len(statuses) != 1 or not statuses.issubset({"IN_PROGRESS", "VERIFIED"}):
        raise SystemExit("RPL001 requirement status missing or inconsistent")
    status = statuses.pop()
    for report in reports:
        ...
        if preserved.get("T2-DMT-001") != "DRAFT" or preserved.get("T2-PAY-002") != "BLOCKED":
    ...
    result = {
        ...
        "decision": "RPL001_VERIFIED_AWAITING_SPONSOR_ACCEPTANCE" if status == "VERIFIED"
                    else "RPL001_CANDIDATE_GREEN_AWAITING_RTM_VERIFICATION",
        "requirementStatus": status, "producers": sorted(producers), "fileCount": len(entries), "files": entries,
        "limitations": [
            "Million-dimension timing is an internal synthetic trend and not a production SLA.",
            "Purchase conversion creates DRAFT only; it does not approve, receive, or change inventory/cost.",
            "T2-RPL-001 is not ACCEPTED until project sponsor confirmation."
        ],
    }
