...
PREFIXES = (
    "lib/infrastructure/local_database/member_cache_store.dart",
    "lib/infrastructure/local_database/s11_member_schema.dart",
)
...
def main() -> None:
    ...
    if not selected:
        raise SystemExit("FLUTTER GATE5C COVERAGE ERROR: no member cache/S11 runtime files found")
    found = sum(value[0] for value in selected.values())
    covered = sum(value[1] for value in selected.values())
    ratio = covered / found
    if ratio < args.minimum:
        raise SystemExit(f"FLUTTER GATE5C COVERAGE ERROR: {ratio:.4f} below {args.minimum:.4f}")
    report = {
        "schemaVersion": "1.0", "scope": "T2 Gate 5C minimal member cache and SQLite V7",
    ...
    print(f"FLUTTER GATE5C COVERAGE OK: lines={covered}/{found} ratio={ratio:.4f}")
