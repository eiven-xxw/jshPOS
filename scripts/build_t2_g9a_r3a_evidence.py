#!/usr/bin/env python3
"""聚合 G9A-R3A 多执行器证据并生成 SHA-256 索引。"""
...
def main() -> int:
    ...
    if missing:
        raise AssertionError(f"缺少 G9A-R3A 证据生产者: {missing}")
    files = [
        {
    ...
    if not files:
        raise AssertionError("G9A-R3A 证据包为空")
    result = {
        "schemaVersion": "1.0",
        "gate": "T2-GATE9B-G9A-R3A",
        "findingId": "G9A-UI-P1-001",
        "batchState": "VERIFIED_CANDIDATE",
        "overallFindingState": "OPEN",
        "evidenceLevel": "INTERNAL_STATIC_AND_SOFTWARE_EXECUTION",
    ...
    print(f"G9A-R3A EVIDENCE OK: producers={len(EXPECTED_PRODUCERS)} files={len(files)}")
    return 0
