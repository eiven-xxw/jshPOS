...
def main() -> None:
    ...
    if missing:
        raise SystemExit("missing T2-SEC-002 producers: " + ", ".join(missing))
    reports = []
    for path in files:
        if path.suffix != ".json":
            continue
    ...
    if len(reports) < 3 or any(report.get("status") != "PASS" for report in reports):
        raise SystemExit("T2-SEC-002 reports missing or non-green")
    for report in reports:
        if report.get("requirementStatus") != "VERIFIED" or len(report.get("closedFindings", [])) != 3:
            raise SystemExit("T2-SEC-002 closure evidence drift")
        if report.get("databaseMigrationsChanged") != 0 or report.get("newBusinessCapabilities") != 0:
            raise SystemExit("T2-SEC-002 scope boundary violated")
        if any(report.get("externalExecution", {}).values()):
            raise SystemExit("T2-SEC-002 external execution boundary violated")
    entries = [{
    ...
    result = {
        ...
        "evidenceCeiling": "INTERNAL_SECURITY_HARDENING_CANDIDATE",
        "decision": "CONDITIONAL_PASS_AWAITING_SPONSOR_ACCEPTANCE",
        "producers": sorted(producers), "fileCount": len(entries), "files": entries,
        "closedFindings": ["G8C-SEC-P0-001", "G8C-SEC-P1-002", "G8C-SEC-P1-003"],
        "externalExecution": reports[0]["externalExecution"],
        "limitations": [
            "T2-SEC-002 remains VERIFIED until project sponsor acceptance.",
            "T2-MTN-001, T2-PERF-002 and T2-RDY-001 remain DRAFT.",
