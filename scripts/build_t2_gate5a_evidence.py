...
def fail(message: str) -> None:
    raise SystemExit(f"T2-GATE5A EVIDENCE ERROR: {message}")
...
def jacoco(root: Path) -> dict[str, float | int]:
    reports = [path for path in root.rglob("jacoco.xml") if "jshpos-promotion" in path.as_posix()]
    if len(reports) != 1:
        fail(f"expected one promotion JaCoCo report, got {len(reports)}")
    ...
    for name, minimum in (("LINE", 0.90), ("BRANCH", 0.85)):
        ...
        if counter is None:
            fail(f"promotion JaCoCo missing {name}")
        missed, covered = int(counter["missed"]), int(counter["covered"])
        ratio = covered / (covered + missed)
        if ratio < minimum:
            fail(f"promotion JaCoCo {name} {ratio:.4f} below {minimum:.2f}")
...
def requirements() -> dict[str, object]:
    ...
    if any(rows[item]["status"] != "VERIFIED" for item in ("T2-PRM-001", "T2-PRM-002", "T2-PRM-003")):
        fail("T2-PRM-001..003 must remain VERIFIED pending sponsor confirmation")
    if rows["T2-PAY-002"]["status"] != "BLOCKED":
        fail("T2-PAY-002 must remain BLOCKED")
    return {"priorAccepted": len(PRIOR_ACCEPTED), "gate5a": "VERIFIED", "paymentSandbox": "BLOCKED"}
...
def main() -> None:
    ...
    tests = {
        ...
        "tenant": surefire(bundle / "tenant"),
        "flutterLinux": flutter_tests(bundle / "pos-linux"),
        "flutterWindows": flutter_tests(bundle / "pos-windows"),
    }
    promotion_coverage = jacoco(bundle / "server")
    ...
    if flutter_coverage.get("lineRatio", 0) < 0.90:
        fail("Flutter promotion line coverage below 90%")
    ...
    if len(attack.get("surfaces", [])) != 21 or any(x.get("result") != "PASS" for x in attack["surfaces"]):
        fail("tenant attack matrix is incomplete")
    vector_reports = list((bundle / "vectors").rglob("fixed-matrix.json"))
    if len(vector_reports) != 1:
        fail("fixed vector matrix is missing")
    vectors = json.loads(vector_reports[0].read_text(encoding="utf-8"))
    if (vectors.get("vectorCount") != 24 or vectors.get("propertyIterations") != 10_000
            or vectors.get("providerNetworkCalls") != 0):
        fail("fixed vector matrix is incomplete or network boundary changed")
    ...
    files = [{"path": path.relative_to(bundle).as_posix(), "size": path.stat().st_size,
              "sha256": digest(path)} for path in sorted(bundle.rglob("*"))
             if path.is_file() and path.name != "t2-gate5a-evidence-index.json"]
    index = {
        "schemaVersion": "1.0", "phase": "T2-GATE5A", "stage": "closure",
        ...
        "branchStart": "63537b5cb7ceeb1fe6b04107b53e2e68941b25ad",
        "commitSha": subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=ROOT, text=True).strip(),
        "requirements": requirements(), "tests": tests,
        "coverage": {"serverPromotion": promotion_coverage, "flutterPromotion": flutter_coverage},
        "tenantAttackSurfaces": 21, "crossRuntimeVectors": 24, "propertyIterations": 10_000,
        "providerNetworkCalls": 0, "apk": {"size": apk[0].stat().st_size, "sha256": digest(apk[0])},
        "externalEvidence": {"sandbox": 0, "realDevice": 0, "pilot": 0}, "files": files,
        "limitations": [
            "Evidence uses two fictional tenants and synthetic promotion, order and refund facts",
            "Payment Provider network, SANDBOX, REAL_DEVICE and PILOT evidence remain zero",
            "Order/Refund owners consume the frozen promotion contract in a later admitted integration step",
            "Synthetic evidence does not establish Alpha, pilot readiness or commercial usability",
        ],
    }
    (bundle / "t2-gate5a-evidence-index.json").write_text(
        json.dumps(index, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(f"T2-GATE5A EVIDENCE OK: files={len(files)} serverTests={tests['server']['tests']} "
          "vectors=24 property=10000 paymentNetwork=0")
