#!/usr/bin/env python3
"""聚合 RPT-INVENTORY V89 索引完整 CI 制品摘要。"""
...
def main() -> None:
    ...
    result_file = next((path for path in files if path.name == "rpt-inventory-keyset-results.json"), None)
    red_file = next((path for path in files if path.name == "v88-red-plan.json"), None)
    if result_file is None or red_file is None:
        raise SystemExit("missing V88 red or V89 result")
    ...
    green = all(not item["fullScanObserved"] and not item["filesortObserved"]
                and item["approvedIndexObserved"] and not item["indexCrRequired"]
                and item["twelveFieldConservationPassed"] for item in plans)
